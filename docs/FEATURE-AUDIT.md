...
### 1-3. イベント間の関連自動リンク【**導出部は round 33 で解決** / 永続化部は要 ADR】

> **round 33 の対応**: 本項の「類似するが別の記事を繋ぐ機構が無い」という穴のうち、
> **導出(算出して見せる)部分は解決済み**。詳細モーダルに関連アイテムを最大3件提示し、
> クリックで辿れる(SPEC §10.22、`async function relatedEvents`)。
> **ただし `links[]` への永続化は行っていない** — 下記「注意」が ADR ゲートにしているのは
> links[] の**意味論変更**であり、描画時の導出はそれに当たらないため(Falsifier Watch と同じ
> 非永続パターン)。**永続的な関連リンク生成は引き続き ADR 待ちで未対応のまま。**
...
- **`normalizeUrl` の正規化不足**(round 57 で**記述を実測に基づき訂正**): 残る未対応は
  **追加トラッカー(`ref`/`igshid` 等)のみ**。
  **訂正**: 旧記述は「ホスト大文字小文字・末尾スラッシュ」も未正規化としていたが、実測すると:
  - **ホスト大小文字は既に正規化済み** — `new URL().toString()` が host を自動で小文字化する
    (`https://Example.com/a` と `https://example.com/a` は現状でも同一 URL に正規化され dedup される)。
    誤った前提で「修正」に着手すると、下記のハッシュ不一致リスクを**利得ゼロで**踏むことになる。
  - **末尾スラッシュは仕様上別 URL** — `/a/` と `/a` は異なるリソースを指しうるため、
    同一視すると別ページを誤って統合する危険がある。**現状維持が正しい**可能性が高く、
    「不足」ではなく設計判断として扱うべき。
  - `http` と `https` も同様に別 URL(内容が同じとは限らない)。
  **注意(変更時)**: 正規化の変更は既存保存イベントとのハッシュ不一致(一時的な重複窓)を
  生むため、着手時はタイトル jaccard フォールバックの救済範囲を確認してから。
  アンカー: `function normalizeUrl` / `tests/normalize-url.test.mjs`
