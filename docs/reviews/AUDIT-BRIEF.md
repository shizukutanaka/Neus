...
- **テスト**: `npm test`(vitest)。**件数はここに書かない** — 手で同期する数字は必ず腐るため
  (round 44 時点で一度同期し直したが再び陳腐化した)。現在値は `npm test` の出力を見ること。
  バグ修正は再現テストを先に足す。
...
## 4. 記録先(実装まで進めた場合)
`SPEC.md` の `### 10.N 第M次監査` を新設(**次の番号は `grep -E '^### 10\.[0-9]+' SPEC.md | tail -1`
で確認**。ここに固定値を書くと必ず陳腐化する)/ `CHANGELOG.md` の
