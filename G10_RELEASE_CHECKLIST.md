...
| G10.07 | ベータ確認(主要フロー全動作・クラッシュゼロ・主観評価 ≥ 4/5) | **CONDITIONAL PASS** | 要件を3分解し、機械検証できる2つを自動化: **主要フロー動作**と**クラッシュゼロ**(pageerror/console.error 監視)。round 67 で除外リスト自体を問い直し、**RSS取得(#2)・キーボード(#15〜#18)・バックアップ往復(#19/#20)を追加で機械化**。`DEPLOY.md` STEP 7 で人手が残るのは **#5 / #7 / #8 / #9 / #16v**(実ディレクトリ選択・OS統合・ブラウザUI)と**主観評価**のみ。件数は `tests/docs-no-frozen-counts.test.mjs` が STEP 7 の表と突き合わせる |
