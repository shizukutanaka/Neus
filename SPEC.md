...
### 10.70 第68次監査 (round 81) — 開けなかったら何が起きるか

round 80 で「書けなくなったら何が起きるか」を問うたので、その**手前**を問うた。実欠陥が2つ出た。

#### 1. `blocked` を拾っておらず、画面が空のまま永久に待っていた

`indexedDB.open()` の終わり方は success / error の2つではなく、**`blocked` を含む3つ**。
別タブが古い版の接続を握ったまま新しいタブが版を上げようとすると `blocked` が発火し、
success も error も来ない。拾っていなかったので **Promise は永久に未解決**だった。
...
**直し方は報告ではなく消去を選んだ**: 接続を持っている側に `onversionchange` を付けて自分から
手放させれば、相手の `blocked` は**ひとりでに解ける**。`onblocked` 側の通知は、相手が古い
ビルド(= `onversionchange` を持たない)だったときの保険として残す。`blocked` で **reject
しない**のは、相手のタブが閉じれば open は続行するため — 待ちは残して理由だけ伝える。

#### 2. 原因を分けずに「全データを消すか」と訊いていた

open 失敗時、アプリは常に「データを削除して再生成しますか?」と訊いていた。しかし**保存領域
そのものが使えない**とき(プライベートウィンドウ / サイトデータのブロック / 権限拒否)は
削除しても直らない。**実測: 承諾 → `deleteDatabase` → 再 open → 同じ失敗。**

つまり**何の役にも立たない全データ消去に同意させていた**。後で設定を戻したときに戻ってくる
はずだったデータは、そのとき既に無い。破壊的な手当ては、それが**効きうる原因**に限る
(`isRecreatable`)。判定は**許可リスト**方式にした — 判定を誤ったとき、こちら側に倒せば
失うのは修復の選択肢だけだが、逆に倒すと利用者のデータが消える。

#### 3. 具体的な説明が総称メッセージに上書きされていた

テスト作成中に見つかった。原因を特定して出した通知を、上流の `catch` が
`初期化に失敗しました: denied` で**塗り潰していた**。通知は1つしか出せないので後勝ちになり、
**役に立たない方が残る**。説明済みの印を付けて、総称側が譲るようにした。

- 変更: `index.html`(`onblocked` / `onversionchange` / `isRecreatable` / 通知の優先)、
  `tests/browser-db-open-failure.spec.mjs`(新規7件)。

### 10.71 第69次監査 (round 82) — 何も拒まれていないのに黙っていた箇所

round 80/81 は「プラットフォームが**拒んだ**とき何と言うか」を問うた。その系統の最後に、
**何も拒まれていないのに黙っている**箇所が残っていた。

この製品の約束は「あなたのデータは端末の中だけにある」。ところが**端末にあることは残ることを
意味しない**。`navigator.storage.persist()` が許可されていなければ、ブラウザは容量逼迫時に
**このオリジンのデータを丸ごと退避(削除)しうる**。
...
**設計の判断を2つ明示する**:
1. **起動時のトーストにはしない。** 毎回出れば読まれなくなるし、その瞬間に打てる手も無い。
   **容量を見に来た場所**に、状態と**次にできること**(インストールで改善する)を並べて置く。
2. **「判定不能」を「未永続化」と混ぜない。** `navigator.storage.persisted` を持たないブラウザ
   では `null` を返し、そう表示する。ここで「安全」と言い切るのが3つの答えの中で最悪
   — 根拠なく安心させることになる。

- 変更: `index.html`(`StorageGuard.isPersisted` / STATS 行 / i18n 4キー×2言語)、
  `tests/browser-data-durability.spec.mjs`(新規8件)。旧実装では8件中**7件**が落ちる(唯一通るのは「起動時に通知しない」— 旧実装も通知していなかったため、これは両側で成立する片方向のガード)。

### 10.72 第70次監査 (round 83) — Worker の上限が「読む量」を抑えていなかった

round 80→82 で「プラットフォームが拒んだ / 黙っていた」系統を閉じたので、対象を**唯一の
ネットワーク接点**である `_worker.js`(240行)へ移した。G0 の第一不変条件(Worker は中継のみ・
状態なし)が乗っている場所である。

**監査の結論としては堅牢だった**: SSRF はホップ毎に `validateTarget` を通し、WHATWG 正規化
(10進/8進/16進 IPv4)・IPv4-mapped IPv6・`[::]`・ULA・link-local を網羅。`/json` はホスト
許可リスト、タイムアウトあり、状態なし。**直すべき点はほぼ無かった。**

例外が `readCapped` 1箇所。コメントは「Content-Length が無くても上限を強制する」と書いて
あったが、実装は `arrayBuffer()` で**全部読み切ってから**大きさを見ていた。

**実測**(Content-Length 無しの 40MB チャンク応答):

| 実装 | 返り値 | 上流から実際に引いた量 |
|---|---|---|
| 旧(`arrayBuffer`) | `null`(拒否) | **40.0MB** |
| 新(逐次読み) | `null`(拒否) | **5.1MB**(上限+1チャンク) |

**返り値は同じ**。Worker のメモリ上限は 128MB なので、十分大きな本文は 413 ではなく
**isolate の死(error 1102)**になる。`/rss` は設計上ホスト許可リストを持たない公開
エンドポイントなので任意の URL で到達し、行儀の悪いフィード1つでも起こりうる。
**上限は「中継しない」ためではなく「読まない」ために要る。**

**なぜ見逃されていたか(こちらが本題)**: テストの describe 名は
`readCapped — streaming size enforcement`、ソース不変条件は
`readCapped helper enforces streaming size even without Content-Length`。
**どちらも「streaming」を名乗りながら、4件すべてが返り値しか見ていなかった。**
返り値は新旧で同一なので、名前が主張する性質は**一度も検査されていなかった**。
しかもソース不変条件は `const buf = await res.arrayBuffer()` を**要求**しており、
上限をメモリに対して無力にしている当の形を固定していた。

**直した内容**: 実装を逐次読みにし、上限超過時点で `reader.cancel()` して残りを引かない。
テストは**引いたバイト数**を数えるものを足し(旧実装は 40MB を引いて落ちることを確認済み)、
ソース不変条件は「`getReader()` を使う」「超過時に `cancel()` する」という**性質**を名指しする
形に書き直した。あわせて多チャンク本文がバイト単位で復元されること(手書きの連結が入ったため)と、
上限直下の本文が切り詰められないことも固定。

これは本セッションで繰り返し出た「**検査器に見えない性質**」の一例
(round 66 の古いミラー、round 77 の実行時合成クラス名と同型)。

- 変更: `_worker.js`(`readCapped`)、`tests/worker.test.mjs`(ミラー同期 + 3件追加 +
  ソース不変条件の書き直し)。

### 10.73 第71次監査 (round 84) — 支配的なテスト方式そのものを測る(ADR-0022 起票)

round 66 / 77 / 83 で**同じ形**の見落としが3回出た: **検査器に見えない性質**。とくに round 83 は、
テストが `readCapped` の「streaming」を名乗りながら**返り値しか見ておらず**、しかもアンカーが
`const buf = await res.arrayBuffer()` を**要求**して、streaming を不可能にする形を固定していた。

3回続いたなら、次の1件を待つのではなく**方式そのもの**を測るべきである。

**実測(`tests/*.{test,spec}.mjs` 全走査)**:

| 種別 | 件数 | 割合 |
|---|---:|---:|
| contract(セレクタ / i18n キー / 設定値) | 211 | 20% |
| other short literal | 589 | 57% |
| **mechanism(書き方そのもの)** | **127** | **12%** |
| long literal(どちらにせよ脆い) | 104 | 10% |
| 合計 | **1,031**(93ファイル) | |

**同期税**: 直近40コミット中 **18件(45%)** がアンカー更新を伴った。実装を正しく直すたびに
テストが赤くなる — これは欠陥の検出ではなく、**同じ事実を2箇所に書いていることの帳尻合わせ**。

**判断**: これは93ファイル・1,031アサーションに及ぶ**テスト方式の変更**であり、CLAUDE.md の
「影響大の場合 ADR起票」「ゼロ設計思考 — 3案比較で選定」に該当する。よって
**`docs/adr/ADR-0022` を Proposed で起票し、一括書き換えは行わない**。3案を比較した:

- **案A 現状維持** … 同期税45%が続き、mechanism 系127件が正しいリファクタを妨げ続ける
- **案B `lib/` 切り出しで本物の import** … 根治だが **ADR-0007(単一ファイル・ゼロビルド)の
  否定**にあたり、配布単位・CSPハッシュ・SW キャッシュへ波及する。本 ADR の範囲外
- **案C(推奨)用途で分ける** … contract は残し、**mechanism 系127件だけ**を
  `tests/helpers/from-source.mjs`(round 60、既存)による**性質のテスト**へ段階移行。
  ADR-0007 を触らず、効果の大きい12%に絞れ、ファイル単位で独立しているので途中で止めても壊れない

**1,031件のうち8割は現に機能している**ので、方式変更は採否そのものが設計判断である。
round 83 で実際に誤りを固定していた1件は、その round 内で既に性質を名指しする形へ直してある。

**あわせて修正**: `CLAUDE.md` の Map が `ADR 0001-0019` と**凍った範囲**を書いていた(実際は 0022)。
round 45 の「手で同期する数字は必ずまた壊れる」がプロジェクトの指示書自身に出ていた形なので、
数字を足すのではなく**数字を消した**(`ls docs/adr/` で確認、と書き換え)。

- 変更: `docs/adr/ADR-0022-source-anchor-tests.md`(新規・Proposed)、`CLAUDE.md`(凍結数値の除去)。
  **実装コードの変更なし。テストの一括書き換えもしない。**

### 10.74 第72次監査 (round 85) — 2年後も動くか(FTS索引の規模特性)

「永久に貯め続ける」製品なので、問うべきは**2年後も動くか**である。FTS索引の構築費用と
メモリを実 Chromium で測った(英日混在・400語彙からサンプリング・スニペットは実運用と同じ
500文字上限という現実的なコーパス)。
...
**検索は速いまま**(20,000件で 9.1ms)。効くのは**メモリ**で、1イベントあたり約7KB。
`StorageGuard` はディスクを見張るが、**インメモリ索引には上限が無い**。携帯端末ではタブが
落とされ、利用者には「アプリが勝手に再読み込みされる」と映る。**欠陥ではなく、記録されて
いなかった設計特性**なので測定値として残す(上限の導入は再現率との取引= 設計判断)。

**測定の失敗を1つ記録する**: 最初のコーパスはスニペットを `'x'.repeat(180)` にしていたため
異なり2-gramが1個しか出ず、語彙が238で飽和して**無意味な数字**が出た。データが悪いと測定は
嘘をつく — 本セッションで繰り返し批判してきた形を自分でやったので、そのまま残す。

**却下した「最適化」(測って否定)**: メモリの **68%(136MB中92.8MB)は `eventGrams`**
(doc→grams の逆引き)で、用途は `remove()` だけ。消せば約2倍軽くなると考えたが、
**語彙の大きさを測って否定した**:
...
`add()` は先頭で `remove()` を呼ぶため、全gram走査方式にすると 20,000件の索引付けが
20,000 × 14,423 ≒ **2.9億回**の削除操作になる。`eventGrams` は無駄ではなく、remove/add を
「その文書のgram数」に抑える**負荷を担う構造**だった。**修正は不要**。

**加えたもの**: envelope は「1文書あたりのgram数が有界」に全面的に依存し、それを支えているのは
**取り込み時の文字数上限**(タイトル300 / スニペット500)である。両取り込み経路(RSS / JSON)で
上限が生きていること、`eventText` が `body` を索引しないこと、`eventGrams` が負荷を担って
いることをテストで固定した。round 51 で `capTitle` を入れたのと同じ構図で、当時はタイトル側
だけを見ていた。

- 変更: `tests/fts-scaling-envelope.test.mjs`(新規9件)のみ。**実装コードの変更なし。**

### 10.75 第73次監査 (round 86) — 直したものは本当に届くのか

既存の SW spec は登録・キャッシュ・オフライン・旧キャッシュ削除・クロスオリジン非介入を
押さえていたが、**最も結果の重い性質**が抜けていた:

> **デプロイした新しいコードは、本当に利用者のところへ届くのか。**

ここが壊れていると、本セッションで直した欠陥は**1つも利用者に届かない**。しかも壊れ方が
静かで、開発側からは正常に見える(サーバには新版が置いてある)。**確認していなかったのは
抜けである** — 他のすべての作業の価値がこの1点に乗っているからだ。

**実測(実 SW・実ブラウザ・可変サーバでデプロイを再現)**: `sw.js` はアプリシェルに
stale-while-revalidate を使うため、設計どおり2段階だった。

| 読み込み | 配信される版 |
|---|---|
| デプロイ直後の1回目 | **旧版**(キャッシュを即返し、裏で新版を取得) |
| 2回目 | **新版** |

**1回目が旧版であることも固定した。** そこが「cache-first に戻す」変更で
**静かに永久停止へ変わる境目**だからである。あわせて、
- 連続したデプロイにも追随すること(1回だけの追いつきではない)
- 更新が**キャッシュに書き戻される**こと — オフラインでも新版が出ること。書き戻さないと
  ローカルファーストが最も効くべき場面(オフライン)で更新を失う

**回帰検出を確認**: `sw.js` を cache-first(書き戻し無し)へ戻すと **4件すべてが落ちる**。
`sw.js` はバイト単位で復元済み。

**修正は不要だった。** 更新経路は設計どおり機能している。加えたのは検証だけ。

- 変更: `tests/browser-sw-update.spec.mjs`(新規4件)のみ。**実装コードの変更なし。**

### 10.76 第74次監査 (round 87) — 製品総括と、設計案そのものへの反問

ゴールに**ソクラテス問答法**が加わり、成果物として「長所・短所・改善点の洗い出し」が明示された。
`docs/reviews/SOCRATIC-AUDIT.md` を新設(`AUDIT-BRIEF §3` からポインタ)。

**形式**: この製品は v0.13 で「問い・反証条件・裁決」というソクラテス式探究を実装している。
**その認識論を製品自身に適用する** — 各項目を「問い → 実測 → **反問** → 判定」で書き、
意見ではなく実測(round 番号)に紐づける。反問を置くのは、**肯定的な答えほど検算されない
まま残る**からである。改善点にはマスク5段階の分類と**決定権者**(機械 / 要ADR / owner)を付す。

**反問が実際に効いた例(1-1)**: 「ゼロ送信は検査済みか」に対し
*テストが通ることと性質が成り立つことは同じか?* と問い返すと、**同じではない**。本セッションで
3回、テストが名前どおりの性質を見ていなかった(round 66/77/83)。よって文書の判定は
「テストが緑」ではなく「**旧実装で赤くなることを確認した**」を根拠にする、と範囲を狭めた。

#### 改善点 3-1 の実装と、設計案そのものへの反問

**問い**: 使えない機能を、使えないと分かる前に押させるのは正しいか。
Firefox / Safari では Vault(File System Access API)が**押した後に**失敗していた。

**最初の設計案を、実装前に反問で却下した**。案は「ボタンを `disabled` + `title` で理由表示」。
反問: *その `title` は影響を受ける当の利用者に見えるのか?* — **見えない**。`title` はホバー
前提で、**iOS Safari(影響ブラウザそのもの)では表示されない**。「無効化 + 見えない理由」は
**理由のない死んだボタン**であり、押せばエラーが出る従来より**悪化**する。
設計案が対象環境で機能しない典型で、案ごと捨てた。

**採用案 — 伝える手段を場所ごとに変える**(根拠は「その環境で本当に読めるか」):
- **Vault モーダル**: ボタンは `disabled`。理由は**同じモーダル内の本文テキスト**(`#vault-status`)
  に出るので、ホバーできない環境でも読める
- **カード詳細の VAULT**: **要素ごと出さない**。ここには理由を書ける本文が無く、`disabled`+`title`
  では非対応環境で理由が読めない。機能の存在はヘッダ VAULT → モーダル本文で説明されるので
  発見経路は残る

判定は既存要素(`#vault-status`)の再利用で行い、新しい UI 部品は足していない。
テストにも「**ホバーせずに**理由が読めること」を明記した — 最初の案が満たせなかった条件である。

- 変更: `index.html`(`VAULT_SUPPORTED` / `updateVaultStatus` / 詳細ボタンの条件出力 /
...
### 10.79 第77次監査 (round 90) — 決定のコストを下げる(ADR-0023 起票)

総括の結論は「残るのは**作業ではなく決定**」だった。しかし**「決定が残っている」には2種類ある**:
*調べてから決める*ものと、*決めるだけ*のもの。前者は実質まだ作業が残っているのと同じである。
そこで**調べる部分を全て済ませ、後者へ変換した**。

未裁決4件のうち `/rss` の流量制限だけ ADR が無く、`SOCRATIC-AUDIT` 2-4 に「要ADR」と記録
されているだけだった。**ADR-0023 を起票**(実装はしない。CLAUDE.md が Worker の状態保持を
名指しで要 ADR としているため、起票して止まるのが規約どおりの動き)。

**測定**(意見ではなく設定値と実装から):

| 要素 | 値 |
|---|---|
| 自動同期の間隔 | 1h / 6h / 12h / 24h、既定 **6h** |
| 1 POLL のリクエスト数 | 登録ソース数 |
| 単語収集の追加分 | 有効単語 × 有効フィード(`WORD_FEEDS` は 8 種) |
| 典型例(ソース10・単語5・各4フィード・6h) | **120 リクエスト/日** = 1利用者あたり 100〜200 の桁 |
| `_worker.js` の状態保持箇所 | **0**(KV / DO / caches / グローバル書込 いずれも無し。機械確認) |

**この件の核心は技術ではない**。「直近 N 秒に何回来たか」を知るには状態が要るので、
**G0「Worker は中継のみ・状態保持禁止」と正面から衝突しうる**。ただし G0 の文言は
「**個人データのサーバ送信ゼロ** — Worker は中継のみ」であり、禁止の目的は利用者データの
残存を防ぐことにある。IP 単位の計数がその趣旨に触れるかは**所有者の判断**で、
ADR が代われない点として明記した。

**3案比較と推奨**: A 現状維持 / **B プラットフォーム側のレート制限ルール(推奨)** /
C Worker 内 KV・DO(不採用)。案B を推す理由は、コードの不変条件を壊さず(`_worker.js` は
1行も変わらない)、**可逆**であり(効かなければ設定を消せば戻る)、守りたいのが利用者データ
ではなく**所有者の枠**という資源なので、プラットフォームの機構で守るのが素直だから。

**「やらない」も正当な決定として明記した**。正規利用では問題が出ておらず、
「起きていない問題を直さない」は十分に正当 — その場合は本 ADR を Rejected として記録する。
閾値を決めるなら**正規利用の上限ではなく濫用の下限から引く**べき、とも書いた。

- 変更: `docs/adr/ADR-0023-rss-endpoint-rate-limiting.md`(新規)、
  `SOCRATIC-AUDIT` 2-4 と総括。**実装コードの変更なし**。

### 10.80 第78次監査 (round 91) — 製品の一番の約束に、検査が無かった

ソクラテス問答を**総括自身**へ向けた。`SOCRATIC-AUDIT` 1-1 は「ゼロ送信は**検査可能**」を
長所として挙げている。反問: **その検査はどこにあるのか。**

**どこにも無かった。** 個々の性質は測ってきた(BYOK 未設定でベンダ通信ゼロ = round 69、
Worker の SSRF = round 83)。しかし**通信を丸ごと記録して「これ以外は出ていない」と言う検査**は
一つも存在せず、`page.on('request')` を使う spec は **0件**だった。
**部分の否定をいくら積んでも全体の否定にはならない。** 製品の一番の約束に対する検査が
「そう書いてある」だけだった、というのが本ラウンドの最大の発見である。

**作った検査**(`tests/browser-egress-allowlist.spec.mjs`): 普通の一連の操作
(初回起動 → OPML取込 → POLL → 全ビュー巡回 → 検索 → キーボード操作 → **メモ保存**)を実行し、
アプリ自身のオリジン以外へ出た**全リクエストを記録**して、宣言した許可リストと突き合わせる。

固定した性質:
- 外部オリジンは**許可リストと完全一致**(記録ではなく許可リストなので、増えれば落ちる)
- **利用者が入力したものは一切出ない** — メモ `MYPRIVATENOTE` と検索語 `MYSECRETQUERY` を
  実際に入力し、全リクエストの URL・クエリ・ボディに現れないことを確認
- 中継へ行くのは**フィードURLだけ** — GET のみ、ボディ空、クエリパラメータは `url` **1つだけ**
- **テレメトリ系ホストへの通信ゼロ**(GA / Sentry / Segment / PostHog 等を名指しで検査)
- 許可リストの各行に**理由が書かれている**こと(理由の無い許可は誰も消せなくなる)

**ガードが噛むことの確認**: `index.html` に Google Analytics のピクセルを1行足すと、
`undeclared outbound origin(s): https://www.google-analytics.com` で落ちる。

**実測が生んだ緊張(ADR-0024 起票)**: 外部オリジンは **2つ**あり、1つは利用者が設定した中継、
もう1つは **`fonts.googleapis.com`** だった。送られるのは利用者の *content* ではないが、
**全起動・全利用者について無条件に** IP と「この時刻にこのアプリを開いた」事実が第三者へ渡る。
しかも `README` にも `docs/` にも**その記載が無かった**。

遮断して測ると: **構造は同一(nav 9)・CLS はどちらも 0・FCP はむしろ改善(124ms → 84ms)**。
`display=swap` と実在するフォールバックスタックのおかげで、**壊れるのは字形だけ**。
つまり争点は性能でも正しさでもなく**美観**であり、測っても答えが出ない。よって ADR は
**案を選ばない**。ただし1つだけ選択肢に依らず言えることを書いた —
**「黙って現状を続ける」は3案のどれでもない**。現状維持を採るなら README への明記が伴う。

- 変更: `tests/browser-egress-allowlist.spec.mjs`(新規5件)、
  `docs/adr/ADR-0024-web-font-third-party-egress.md`(新規)、`SOCRATIC-AUDIT` 1-1 の格下げ。
  **実装コードの変更なし**。

### 10.81 第79次監査 (round 92) — 同じ問いを「失敗を隠さない」へ

round 91 は総括 1-1 に「その検査はどこにあるのか」を当て、**無かった**ことを見つけた。
本ラウンドは 1-2「**失敗を隠さない**」に当てる。**答えは同じだった** — round 80/81/82 は
書けない・開けない・保持保証が無いを個別に直したが、「**どこにも黙って握り潰す箇所が無い**」
ことは一度も確認していなかった。
...
| console のみ | 27 |
| 再 throw | 1 |
| **空(何もしない)** | **12**(index.html 11 + sw.js 1)|
| 残り | 代替値を返す / 返り値で伝える(`return url` / `{ok:false,reason}` / `error:'parse'`)|

**空 catch を1件ずつ読んだ結果、握り潰しは1件も無かった。** 全て
(a) 後始末の best-effort(`db.close()` / `t.abort()` / `w.abort()` / `prev?.focus()`)か、
(b) 失敗しても**安全側に倒れる**経路。

特に追跡したのが `syncPrefsToSW` の `cache.put('/__prefs')`。黙って失敗すると sw.js 側は
`prefs?.notify` が false になり**通知しない**。失敗の向きは「望まぬ通知が出る」ではなく
「望んだ通知が出ない」= **fail-closed** で、製品の姿勢と一致している。

**したがって実装は変更しない。** 直すものが無いときに直さないのは、直すのと同じくらい重要である。

**代わりに残したもの**: `tests/silent-catch-allowlist.test.mjs`。空 catch を**理由つき許可リスト**
として固定し、(1) 未登録の空 catch があれば**ファイル・行・守っている文**を名指しで落とす、
(2) 各理由が実質を持つこと、(3) **使われなくなった許可を残さない**(許可リストが一方的に
伸びると「何でも握り潰してよい」に変質するため)を検査する。

**ガードは初回実行で仕事をした**: 手で数えた11件は index.html しか見ておらず、
**sw.js の1件を見落としていた**。ガードがそれを名指しで落とした — **人の目視より機械の網羅が
要る**という、このテストの存在理由そのものである。新規の空 catch を1つ足すと
`index.html:4715 guarding: JSON.parse("x");}` で落ちることも確認済み。

- 変更: `tests/silent-catch-allowlist.test.mjs`(新規5件)、`SOCRATIC-AUDIT` 1-2 の裏付け追記。
  **実装コードの変更なし**。

### 10.82 第80次監査 (round 93) — 検算はどこまで届いていたか

round 91/92 と同じ反問を総括 1-5「**文書が自分を検算する**」へ当てた。既存のガードは3つあり、
どれも**内容**を見ている(件数の固定 / i18n の死にキー / 旧プロジェクト名)。
**参照の健全性**と**採番の整合**は誰も見ていなかった。

**これは机上の心配ではない**。**round 87 で実際に §10.73 を二重に振った**。気づいたのは偶然で、
見落としていれば SPEC は同じ節番号を2つ持ったまま出荷されていた。90ラウンド分の追記を人の目で
守り続けるのは、round 92(手で数えた11件が sw.js の1件を見落とした)で示したとおり無理がある。
...
| ADR | **24** — 全てに status 行あり、番号重複なし |
| 文書からの参照(`tests/` · `scripts/` · ルートファイル · `ADR-00XX` · `§10.XX`)| 壊れ **0** |

**したがって直すものは無い。** 本ラウンドの成果は修正ではなく、
**round 87 の誤りが今後は自動で落ちる**ようになったことである。確認済み:
節番号を重複させると `two sections cannot share a number: expected [ 80 ]` で落ち、
テストファイル名を存在しないものに書き換えると `documents citing files that are not there` で落ちる。

**ADR に status 行を要求する理由**: 状態の書かれていない ADR は**行動できない** —
提案なのか、決定済みなのか、却下されたのかが分からなければ、読んだ人は次に何もできない。
未裁決4件を「決めるだけ」の状態に保つ(round 90)ための前提条件でもある。

- 変更: `tests/docs-integrity.test.mjs`(新規11件)、`SOCRATIC-AUDIT` 1-5 への反問追記。
  **実装コードの変更なし。文書の内容変更もなし。**

### 10.83 第81次監査 (round 94) — 56ラウンド前から古びていたミラー

総括 1-3「**検証の様式**」— 実ソース評価・実ブラウザ・旧実装で赤くなることの確認 — に
同じ反問を当てた: *ミラーはもう残っていないのか。*

**残っていた。しかも古びていた。**

`tests/utils.test.mjs` は冒頭で「Pure functions are copied here … In production they live inside
index.html's ES module」と**production の関数であると主張していた**。しかし `tokenize` は
**round 37 で CJK 字種境界分割を得ており**、コピーはそれ以前の空白分割のままだった:

```
tokenize('Rustの所有権とライフタイム入門')
  ミラー → ["rustの所有権とライフタイム入門"]      ← 分割されない1トークン
  実物   → ["rust","所有権","ライフタイム","入門"]
```

**56ラウンド緑のままだった理由**は round 66 の OPML ミラーと同じ — **コピーを自分自身と
比べていた**うえ、テストの入力が英語で、英語なら両者は一致するからである。

**修正**: `normalizeUrl` / `jaccard` / `tokenize` / `ngrams` を実ソース評価へ置き換え、
偽っていたヘッダを事実に書き換えた。40件のテストは実物に対して全て通る。

**同時に検証して問題が無かったもの**(直さない):
- `entity-kanji` / `entity-recency` の `extractEntities` ミラー2件 — 5入力で実物と**完全一致**
- `escapeHtml` ミラー — 8入力(`<script>` / null / 数値 等)で**完全一致**

**一般的な検出器は作らないと判断した(否定的結果の記録)**。「テスト内で production と同名の
関数を定義している箇所」を機械的に数えると **264件**あるが、その大半は
**実ソース評価のために注入している依存**(`toast` / `uuid` / `verdictOf` 等)か、たまたま名前が
衝突しただけのローカル補助(`n` / `t` / `label`)である。ここにガードを置けば
**264行の許可リスト**が生まれ、誰も読まなくなる — round 92 で「使われなくなった許可を残さない」
と定めた理由そのものに反する。**作れるからといって作るべきではない**。

したがって本ラウンドの成果は、**1件の実ドリフト修正と、2件の検証済み(無修正)、
そして「一般ガードは作らない」という判断の記録**である。ミラーは触れた時に実ソース評価へ
置き換える(round 60 で用意した手順)という既存の運用が、この種の腐敗に対する現実的な答えである。

- 変更: `tests/utils.test.mjs`(ミラー4件 → 実ソース評価、ヘッダの主張を訂正)。
  **実装コードの変更なし。**

### 10.84 第82次監査 (round 95) — 空のアプリで測った数字は、使い込んだアプリの数字ではない

総括 1-4「性能」に反問を当てた: *この測定は誰の端末を写しているのか。*

`browser-vitals.spec.mjs` も `browser-lighthouse-score.spec.mjs` も **何も seed していなかった**
(`indexedDB` への参照が 0)。つまり「Performance = 99」「CWV 全て good」は**初日の空の状態**の
数字であって、1年使った利用者の体験ではない。round 85 は FTS 索引の規模特性を測ったが、
**起動時の体感**は測っていなかった。「永久に貯め続ける」製品でこの穴は物足りない。

**実測(seed 済みプロファイルで再起動)**:

| 保存イベント数 | FCP | CLS | TBT | 初回描画カード数 |
|---:|---:|---:|---:|---:|
| 0 | 64 ms | 0 | 0 ms | 0 |
| 2,000 | 68 ms | 0 | 0 ms | 50 |
| 10,000 | **52 ms** | 0 | 0 ms | 50 |

**劣化しない。** 理由は2つとも既存の設計判断である:
- **round 28** で FTS/TagLearner/StorageGuard の全件スキャンを**初回描画の後ろ**へ回した
- 初回描画は `listEvents({limit})` で、`CONFIG.maxViewItems` により **50枚で頭打ち**

**したがって 1-4 は規模に対しても成り立ち、直すものは無い。** 追加したのは、この性質が
将来の変更で静かに失われないための検査(10,000件 seed 後の CWV / 初回描画の件数上限 /
重いスキャンが描画より後にあること)。

**自分の書いたガードが機能しなかった件を記録する**。最初の順序アサーションは
「最初の `Perf.mark('render')` を取り、その後に `FTSIndex.rebuild()` があること」を見ていた。
ところが `Perf.mark('render')` は**2箇所**あり(オンボーディング側と起動側)、起動側の描画の
**前に** rebuild を注入しても、オンボーディング側のマークより後ろに正規の rebuild が残るため
**テストは通ってしまった**。書いた当のリグレッションを検出できないガードで、
**本セッションで繰り返し批判してきた形を自分でやった**。起動ブロックを切り出して
「描画前にスキャンが現れないこと」を見る形へ書き直し、注入で
`FTSIndex.rebuild() runs before the first paint` と落ちることを確認した。

- 変更: `tests/browser-vitals.spec.mjs`(+3件、うち1件は書き直し)。**実装コードの変更なし**。

### 10.85 第83次監査 (round 96) — 自分のガードを壊してみる

round 95 で、**自分の書いたガードが、書いた当のリグレッションを検出しなかった**。
そこから出る問いは一つ: **他のガードは本当に落ちるのか。**

round 88–95 で追加したガードのうち、明示的に「壊して落ちること」を確認したものと、
していないものを洗い出した。未確認は **round 89 で「文字列検査 → 実測」へ移した3件**。
壊して確かめた結果、**1件は落ちなかった**。

#### `the holding tab yields so the block resolves itself` — アプリを一切見ていなかった

round 89 のこのテストは `page.evaluate` の中で**自前のデータベースを開き、自前の
`onversionchange` を仕掛けて** IndexedDB の挙動を観測していた。つまり
**プラットフォームの性質**を、**この製品の性質**であるかのように書いていた。

**`index.html` から `db.onversionchange` を丸ごと削除しても、テストは緑のままだった。**

「shape から実測へ移した」つもりの変更が、**測る対象を間違えていた**。round 89 の
自己評価そのものが誤っていたことになる。

#### 直し方 — アプリ自身の接続に圧力をかけ、アプリの反応を見る

アプリを起動させて**アプリ自身が握っている `neus-v1`** に対し、ページから
より新しい版で `indexedDB.open` する。観測するのは**アプリが出す通知**
「another tab updated Neus — reload this tab」である。

| index.html | upgrade 完了 | アプリの通知 |
|---|---|---|
| そのまま | 完了 | **出る** |
| `onversionchange` 削除 | **完了** | **出ない** |

**「upgrade が完了すること」は主張しないことにした。** 実測すると**ハンドラの有無に関わらず
完了する**ので、それは Chromium についての証拠であって本製品についての証拠ではない。
主張すれば証明に見えて何も証明しない — **round 89 版がまさにそれで通っていた**。
その旨をテスト本文に明記した。

削除すると `the app must notice its connection was taken and say so` で落ち、
戻すと通ることを確認済み。

**残る2件は確認済みで問題なし**: 「blocked 後に開き直せば起動する」は実際にアプリを読み込んで
本文が描画されることを見ており、「復旧可能な失敗では修復を提案する」は `VersionError` を注入して
実際にモーダルが出ることを見ている。どちらもアプリを観測している。

**得られた規律**: 「実測に移した」と言うだけでは足りない。**何を測っているのかを問う**必要がある。
プラットフォームの挙動を測って製品の挙動だと述べるのは、文字列を測るより**悪い** —
実測しているように見えるぶん、疑われにくいからである。

- 変更: `tests/browser-db-open-failure.spec.mjs`(1件を書き直し、1件を削除)。
  **実装コードの変更なし**。

### 10.86 第84次監査 (round 97) — 一番大事な経路のテストが、実装から切れかけていた

round 95/96 で**自分のガードが2件空振りしていた**ことを踏まえ、次に問うべきは
「**最も重い性質を守っているテストは大丈夫か**」である。CLAUDE.md が human-in-the-loop に
指定している経路 — **マスターパスフレーズによる API キーの暗号化** — を見た。

`tests/crypto.test.mjs` は**手写しのミラー**であり、パラメータを**直書き**していた:

```
const PBKDF2_ITERATIONS = 300000;   // 実装は CONFIG.pbkdf2Iterations
const IV_LEN = 12;                  // 実装は CONFIG.ivLen
```

**現在の値は一致している。** 問題は、**一致を保つものが何も無い**ことだった。

**しかもこれは差し迫った危険だった**。`ADR-0021` は**まさにこの反復回数を変える提案**
(300k → OWASP の 600k)である。承認して `CONFIG.pbkdf2Iterations` を書き換えれば、
このファイルは**古い値のまま**動き続け、**13件全てが緑のまま、製品が使っていない
パラメータを検証する**ことになる。製品で最も重い経路のテストが、静かに実装から外れる。

**修正**: パラメータを `index.html` から読む。アルゴリズム本体は適応せざるを得ないため
(実 `encrypt` は salt を IndexedDB から取るのでここでは動かない)、適応では検証できない選択を
アンカーで固定した — どの CONFIG キーを使うか、そして**リテラルを使っていないこと**。

**2方向で確認した**:

| 操作 | 結果 |
|---|---|
| ADR-0021 を承認したつもりで `pbkdf2Iterations:600000` にする | テストは**追随**(13件緑、ファイルに 300000 は残らない) |
| `iterations:CONFIG.pbkdf2Iterations` をリテラルへ戻す(結合を切る) | `iterations must come from CONFIG` で**落ちる** |

**この系統(round 94→97)が示すもの**: ミラーの危険は「古びること」だけではない。
**まだ古びていないミラーも、古びる経路が塞がれていなければ同じ**である。round 94 は既に
古びたものを見つけ、本ラウンドは**これから古びるはずだったもの**を、
それを古びさせる決定が下される前に見つけた。

- 変更: `tests/crypto.test.mjs`(パラメータを実ソースから読む + アンカー4件)。
  **実装コードの変更なし**。
