...
### Added
- **暗号テストのパラメータ直書きを解消し、ADR-0021 承認時の静かな乖離を防いだ(round 97)**: round 95/96 で自分のガードが2件空振りしていたため、**最も重い性質**を守るテストを見た — CLAUDE.md が human-in-the-loop に指定する**マスターパスフレーズによる API キー暗号化**。`tests/crypto.test.mjs` は手写しミラーで、`PBKDF2_ITERATIONS = 300000` / `IV_LEN = 12` を**直書き**していた(実装は `CONFIG.pbkdf2Iterations` / `CONFIG.ivLen`)。**値は一致していたが、一致を保つものが無かった。** しかも **ADR-0021 はまさにこの反復回数を変える提案**(300k → 600k)で、承認すればこのファイルは古い値のまま緑を出し続け、**製品が使っていないパラメータを検証する**ことになっていた。パラメータを `index.html` から読むようにし、適応では検証できない選択(どの CONFIG キーを使うか / **リテラルを使っていないこと**)をアンカーで固定。**2方向で確認**: `600000` へ変えるとテストは追随し、`iterations:CONFIG.pbkdf2Iterations` をリテラルに戻すと `iterations must come from CONFIG` で落ちる / Bound the crypto tests' parameters to the real CONFIG — they had been hard-coded, and accepting ADR-0021 would have left the most consequential path in the app tested against a value it no longer used
- **自分のガードを壊して回り、アプリを一切見ていなかった1件を書き直した(round 96)**: round 95 で**自分のガードが書いた当のリグレッションを検出しなかった**ため、他のガードも壊して確かめた。未確認だったのは round 89 で「文字列検査 → 実測」へ移した3件で、うち**1件が落ちなかった**。`the holding tab yields so the block resolves itself` は `page.evaluate` 内で**自前のデータベースを開き自前の `onversionchange` を仕掛けて** IndexedDB の挙動を観測しており、**`index.html` から `db.onversionchange` を丸ごと削除しても緑のまま**だった — **プラットフォームの性質を製品の性質として書いていた**。アプリ自身が握る `neus-v1` により新しい版で `open` し、**アプリが出す通知**を観測する形へ書き直した(削除すると落ち、戻すと通ることを確認)。あわせて「**upgrade が完了すること**」の主張を**削除**した — 実測ではハンドラの有無に関わらず完了するので Chromium についての証拠でしかなく、主張すれば証明に見えて何も証明しない。残る2件は実際にアプリを観測しており問題なし / Broke my own guards on purpose and found one that never watched the app at all — it observed IndexedDB's behaviour and reported it as the product's
- **性能の測定を「使い込んだプロファイル」でも行うようにした(round 95)**: 総括 1-4 への反問「**この測定は誰の端末を写しているのか**」— CWV も Lighthouse 相当も **何も seed していなかった**(`indexedDB` 参照 0)ので、「Performance = 99」は**初日の空の状態**の数字だった。seed して測り直すと **10,000件でも FCP 52ms / CLS 0 / TBT 0ms**(空:64ms、2,000件:68ms)、初回描画は **50枚で頭打ち**。**劣化しない** — round 28 の「重いスキャンを初回描画の後ろへ」と `CONFIG.maxViewItems` が効いているため。**よって直すものは無く**、性質が将来静かに失われないための検査を追加した。**自分のガードが機能しなかった件も記録**: 最初の順序アサーションは `Perf.mark('render')` が2箇所ある(オンボーディング側と起動側)ことを見落とし、**書いた当のリグレッションを注入しても通ってしまった**。起動ブロックを切り出す形へ書き直し、注入で落ちることを確認済み / Extended the performance measurement to a used profile — it holds at 10,000 events — and fixed an ordering guard that failed to catch the regression it was written for
- **文書の参照と採番の整合を機械が見張るようにした(round 93)**: 総括 1-5「文書が自分を検算する」へ同じ反問を当てた —「**その検算はどこまで届いているのか**」。既存3ガードは全て**内容**を見ており、**参照の健全性**と**採番の整合**は誰も見ていなかった。**机上の心配ではない: round 87 で実際に §10.73 を二重に振った**(気づいたのは偶然)。実測はいずれも健全 — §10 監査節 **78**(節番号・監査序数・round 番号すべて重複なし、昇順、欠番なし)、ADR **24**(全てに status 行、番号重複なし)、文書からの参照(`tests/` · `scripts/` · ルートファイル · `ADR-00XX` · `§10.XX`)の壊れ **0**。**よって直すものは無い** — 成果は修正ではなく、**round 87 の誤りが今後は自動で落ちる**こと。節番号を重複させると `two sections cannot share a number` で、存在しないテストファイルを参照すると `documents citing files that are not there` で落ちることを確認済み / Made document references and audit-log numbering machine-checked, so the duplicate section number I made in round 87 would now fail rather than depend on being noticed
- **「失敗を隠さない」を空 catch の許可リストとして固定した(round 92)**: round 91 と同じ問いを総括 1-2 に当てた —「その検査はどこにあるのか」。**答えは同じく「無い」**だった。round 80/81/82 は個別に直したが「**どこにも黙って握り潰す箇所が無い**」ことは一度も確認していなかった。全 `catch` を機械分類(98件: 通知 **32** / console のみ 27 / 再 throw 1 / **空 12** / 残りは代替値か返り値)。**空 catch を1件ずつ読んだ結果、握り潰しは1件も無かった** — 全て後始末の best-effort か **fail-closed** な経路(例: prefs 書込が失敗すると SW は**通知しない**ので、失敗の向きは「望まぬ通知」ではなく「望んだ通知が出ない」)。**よって実装は変更しない**。代わりに理由つき許可リストとして固定し、未登録の空 catch を**ファイル・行・守っている文**で名指しし、理由が実質を持つこと、**使われなくなった許可を残さない**ことも検査する(許可リストが一方的に伸びると「何でも握り潰してよい」に変質するため)。**ガードは初回実行で、手で数えた11件が index.html しか見ておらず sw.js の1件を見落としていたことを検出した** / Pinned "failures are not hidden" as a reasoned allow-list of empty catches — the sweep found no swallowing, and the guard immediately caught one I had missed by hand
- **「個人データのサーバー送信ゼロ」を通信そのものを記録して固定した(round 91)**: 総括 1-1 は「ゼロ送信は**検査可能**」を長所に挙げていたが、反問「**その検査はどこにあるのか**」の答えは**どこにも無かった** — 個々の性質は測っていても、**通信を丸ごと記録して「これ以外は出ていない」と言う検査**は一つも無く、`page.on('request')` を使う spec は **0件**だった。**部分の否定を積んでも全体の否定にはならない。** 普通の一連の操作(初回起動 → OPML取込 → POLL → 全ビュー巡回 → 検索 → メモ保存)で外部へ出た**全リクエストを記録**し、許可リストと完全一致すること・**利用者が入力したメモと検索語が一切出ない**こと・中継へ行くのは**フィードURLだけ**(GET のみ、ボディ空、パラメータは `url` 1つ)・**テレメトリ系ホストへの通信ゼロ**を固定。GA のピクセルを1行足すと origin を名指しで落ちることを確認済み。実測の副産物として **Google Fonts への無条件接続**が可視化され、遮断しても**構造同一・CLS 0・FCP はむしろ改善**(124ms → 84ms)と分かったため **ADR-0024** を起票(争点は美観のみなので案は選ばない。ただし「**黙って現状を続ける**」は選択肢ではないと明記)/ Pinned the product's headline promise by recording every outbound request, and surfaced an unconditional third-party font connection that was documented nowhere
- **`/rss` 流量制限の ADR-0023 を起票し、未裁決4件を全て「決めるだけ」の状態にした(round 90)**: 総括の結論は「残るのは作業ではなく決定」だったが、**「決定が残っている」には2種類ある** — *調べてから決める*ものと*決めるだけ*のもの。前者は実質まだ作業が残っているのと同じなので、**調べる部分を全て済ませて後者へ変換した**。未裁決4件のうち `/rss` 流量制限だけ ADR が無かった。実測: 正規利用は**1利用者あたり 1日 100〜200 リクエスト**(既定 6h 間隔・ソース10・単語5・各4フィードで 120/日)、`_worker.js` の状態保持箇所は **0**。**核心は技術ではない** — 「直近 N 秒に何回来たか」を知るには状態が要り、G0「Worker は中継のみ・状態保持禁止」と衝突しうる。ただし G0 の目的は利用者データの残存防止なので、IP 単位の計数がその趣旨に触れるかは**所有者の判断**であり ADR が代われないと明記。3案比較(A 現状維持 / **B プラットフォーム側ルール = 推奨** / C Worker 内 KV・DO = 不採用)。**「やらない」も正当な決定**として明記し、閾値は正規利用の上限ではなく**濫用の下限**から引くべきと記した。実装はしない(CLAUDE.md が Worker の状態保持を名指しで要 ADR としており、起票して止まるのが規約どおり)/ Filed ADR-0023 so every pending design decision now has its research done — what remains is preference, not investigation
- **browser spec にも同じ問いを当て、3件を文字列検査から実測へ移した(round 89)**: round 88 の掃討は unit テストだけを見ていたが、**実行時の挙動を最も声高に主張するのは browser spec の方**。「`page` を受け取らず `index.html` の文字列だけを検査している test」は **15件**(うち2件は既に `shape` と名乗り済み)。**測れる3件を実測へ**: (1)「保持側が譲るのでブロックは自然に解ける」→ 実 IndexedDB で**別接続の upgrade が実際に完了**することを確認し、**制御群**として「譲らなければ blocked のまま完了しない」も測る、(2)「blocked は reject しないので開き直せば起動する」→ `blocked` を配送してから success させ、**アプリが実際に起動する**ことを確認、(3)「復旧可能な失敗では修復を提案する」→ `VersionError` を注入して**確認モーダルが実際に出る**ことを確認(round 81 で破壊的経路を狭めたので、**狭めすぎて修復手段を消していない**ことを測る必要があった)。残る10件は構造的主張なのでタイトルに `(shape)` を付け、本文以上を約束しないようにした。**無標のソースのみ browser test はゼロ**になった / Moved three browser-spec claims from string matching to measurement, and labelled the rest as shape so no title promises more than its body checks
- **製品総括 `docs/reviews/SOCRATIC-AUDIT.md` を新設し、Vault 非対応を押す前に伝えるようにした(round 87)**: 長所・短所・改善点を**ソクラテス問答形式**(問い → 実測 → **反問** → 判定)で整理。改善点にはマスク5段階の分類と**決定権者**(機械 / 要ADR / owner)を付す。反問を置くのは肯定的な答えほど検算されないまま残るためで、実際に「ゼロ送信は検査済みか」→「テストが通ることと性質が成り立つことは同じか」で判定の範囲が狭まった(本セッションで3回、テストが名前どおりの性質を見ていなかった)。あわせて指摘の1件を実装 — Firefox / Safari では Vault が**押した後に**失敗していたのを、押す前に理由が読めるようにした。**最初の設計案(`disabled` + `title`)は実装前に反問で却下**: `title` はホバー前提で **iOS Safari(影響ブラウザそのもの)では表示されない**ため、「無効化 + 見えない理由」は押せばエラーが出る従来より悪化する。採用案は場所ごとに手段を変え、モーダルは**本文テキスト**で理由を出し(ホバー不要)、理由を書ける本文が無いカード詳細の VAULT ボタンは**要素ごと出さない**(発見経路はヘッダ → モーダルに残る)/ Added a Socratic product audit, and made Vault state why it is unavailable before it is clicked — after rejecting a tooltip-based design that would be invisible on the very browsers it targets
- **デプロイした新版が利用者へ届くことを実ブラウザで固定(round 86)**: 既存の SW spec は登録・キャッシュ・オフライン・旧キャッシュ削除を押さえていたが、**最も結果の重い性質** — 「**デプロイした新しいコードは本当に利用者へ届くのか**」 — が抜けていた。ここが壊れると本セッションで直した欠陥は**1つも届かず**、しかもサーバには新版があるので開発側からは正常に見える。可変サーバでデプロイを再現して実測した結果、`sw.js` の stale-while-revalidate は設計どおり**2段階**(1回目=旧版・2回目=新版)で機能しており、**修正は不要**。加えたのは検証で、**1回目が旧版であることも固定**した(そこが「cache-first に戻す」変更で静かに永久停止へ変わる境目のため)。連続デプロイへの追随と、更新が**キャッシュへ書き戻される**こと(= オフラインでも新版が出る。書き戻さないとローカルファーストが最も効くべき場面で更新を失う)も固定。`sw.js` を cache-first へ戻すと**4件すべてが落ちる**ことを確認済み / Pinned that a deployed update actually reaches the reader — the property every other fix depends on, and the one the SW specs had not checked
- **FTS索引の規模特性を実測し、それを支える境界をテストで固定(round 85)**: 「永久に貯め続ける」製品なので**2年後も動くか**を問うた。実 Chromium で測った結果、**検索は速いまま**(20,000件で 9.1ms)だが**メモリが効く** — 2,000件 +13.2MB / 10,000件 +56.9MB / **20,000件 +139.9MB**(1イベント約7KB)。`StorageGuard` はディスクを見張るが**インメモリ索引には上限が無い**ため、携帯端末ではタブが落とされうる。**欠陥ではなく記録されていなかった設計特性**として測定値を残した(上限導入は再現率との取引=設計判断)。**却下した最適化**: メモリの68%は `eventGrams`(doc→grams 逆引き、用途は `remove()` のみ)で消せば約2倍軽くなると考えたが、語彙を測ると漢字2000+かなで**14,423 gram** に達し、`add()` が先頭で `remove()` を呼ぶため全gram走査にすると20,000件の索引付けが**2.9億回**の削除操作になる — **負荷を担う構造**であり修正不要。envelope が依存する取り込み時の文字数上限(タイトル300 / スニペット500)が**両経路で**生きていること、`eventText` が `body` を索引しないことを固定した / Measured the FTS index scaling envelope, recorded it, and pinned the ingest caps it depends on — including a rejected optimization that measurement showed would be 290M operations worse
- **ADR-0022 を起票: ソースアンカー方式の位置づけ(Proposed / 未実装)(round 84)**: round 66 / 77 / 83 で**同じ形**の見落としが3回出た(**検査器に見えない性質**)。とくに round 83 はテストが「streaming」を名乗りながら**返り値しか見ておらず**、アンカーが `const buf = await res.arrayBuffer()` を**要求**して streaming を不可能にする形を固定していた。3回続いたので方式そのものを測った — **1,031アサーション / 93ファイル**、内訳は contract 211(20%)・other 589(57%)・**mechanism 127(12%)**・long literal 104(10%)。同期税は**直近40コミット中18件(45%)**。これは93ファイルに及ぶテスト方式の変更なので、CLAUDE.md「影響大の場合 ADR起票」「3案比較」に従い **ADR を Proposed で起票し、一括書き換えはしない**(案A 現状維持 / 案B `lib/` 切り出し=ADR-0007 の否定 / **案C 推奨: mechanism 系127件だけを既存の `from-source.mjs` による性質テストへ段階移行**)。1,031件の8割は現に機能しており、採否そのものが設計判断であるため / Filed ADR-0022 measuring the source-anchor test convention (1,031 assertions, 45% of recent commits paying a sync tax) and proposing a scoped migration — deliberately not applied
- **データが「残る保証があるか」を STATS に表示(round 82)**: この製品の約束は「データは端末の中だけにある」だが、**端末にあることは残ることを意味しない** — `navigator.storage.persist()` が許可されていなければ、ブラウザは容量逼迫時に**このオリジンのデータを丸ごと退避(削除)しうる**。実測(通常の Chromium): `persisted()` は **false**、STATS は `Storage | 0.0MB / 0.9GB` のみ、**永続化への言及はどこにも無い**(console にすら出ない)。つまり**データが消えうる状態を利用者は知る術がなかった** — 容量は見せて保持は見せていない、という抜け。設計判断を2つ: (1) **起動時トーストにはしない**(毎回出れば読まれず、その瞬間に打てる手も無い)。容量を見に来た場所に状態と**次にできること**(インストールで改善)を並べる。(2) **「判定不能」を「未永続化」と混ぜない** — `persisted` を持たないブラウザでは `null` を返しそう表示する。根拠なく「安全」と言うのが3つの答えの中で最悪だから / Surfaced whether the browser has actually promised to keep the data, next to the storage figures in STATS
...
### Fixed
- **56ラウンド前から古びていたミラーを実ソース評価へ置き換えた(round 94)**: `tests/utils.test.mjs` は冒頭で「production の関数のコピー」と**主張していた**が、`tokenize` は **round 37 の CJK 字種境界分割以前のまま**だった — `tokenize('Rustの所有権とライフタイム入門')` はミラーが `["rustの所有権とライフタイム入門"]`(分割されない1トークン)、実物は `["rust","所有権","ライフタイム","入門"]`。**56ラウンド緑だった理由**は round 66 の OPML ミラーと同じで、**コピーを自分自身と比べており**、テスト入力が英語なら両者が一致するから。`normalizeUrl` / `jaccard` / `tokenize` / `ngrams` を実ソース評価へ置き換え、偽っていたヘッダを訂正(40件全通過)。**同時に検証して無修正としたもの**: `extractEntities` ミラー2件(5入力で完全一致)と `escapeHtml`(8入力で完全一致)。**一般的な検出器は作らないと判断**した — 同名定義は 264件あるが大半は実ソース評価のための注入依存(`toast`/`uuid`/`verdictOf`)か名前衝突(`n`/`t`/`label`)で、ガード化すれば **264行の誰も読まない許可リスト**になり、round 92 で定めた方針に反する。**作れるからといって作るべきではない** / Replaced a mirror that had been stale for 56 rounds with real-source evaluation, verified two others as faithful, and deliberately did not build a general detector that would have produced an unreadable allow-list
- **復元が途中で失敗すると「既存データは保持されています」と表示しながら全データを消していた(round 88)**: `Store.replaceAll` は単一 tx で `clear()` → `put()` するが、**`put()` が同期で投げる例外**(キー評価不能 / clone 不能)は executor から抜けて Promise を reject する一方、**tx は abort されず auto-commit** する — IndexedDB が abort するのは非同期の request error だけ。実測: 既存3件が「復元に失敗(既存データは保持)」の表示とともに backup の1件目だけに置き換わった。**到達可能**: 復元は events と words を検証するが **sources は検証していなかった**ため、id の無いソースが1件でも混じった backup JSON(手編集 / 部分破損)で DataError が **events を消した後に**投げる。`replaceAll` は同期例外を `t.abort()` に変え、復元は sources も消す前に検証するようにした(`validSource`)。発見の経緯: 「atomic」と名乗るテストが原子性の**形**しか見ておらず、実測に切り替えた途端に出た(SOCRATIC-AUDIT 3-2 の実行) / Fixed restore committing a half-done replacement while reporting the data preserved — synchronous put() throws never aborted the transaction, and sources were the one store never validated
- **Worker の 5MB 上限が「読む量」を抑えておらず、巨大な応答で isolate が死にえた(round 83)**: `readCapped` のコメントは「Content-Length が無くても上限を強制する」と書いてあったが、実装は `arrayBuffer()` で**全部読み切ってから**大きさを見ていた。実測(CL 無しの 40MB チャンク応答): 旧は **40.0MB を引いてから**拒否、新(逐次読み)は **5.1MB** で打ち切り。**返り値は新旧同一**。Worker のメモリ上限は 128MB なので十分大きな本文は 413 ではなく **isolate の死(error 1102)**になり、`/rss` は設計上ホスト許可リストを持たない公開エンドポイントなので任意の URL で到達する(行儀の悪いフィード1つでも起こりうる)。**上限は「中継しない」ためではなく「読まない」ために要る。** 見逃されていた理由の方が重要で、テストの describe 名は `streaming size enforcement`、ソース不変条件は `enforces streaming size` だったにもかかわらず、**4件すべてが返り値しか見ておらず**、しかもソース不変条件は `const buf = await res.arrayBuffer()` を**要求**して当の形を固定していた。**引いたバイト数**を数えるテストを追加し(旧実装が落ちることを確認)、不変条件は「`getReader()` を使う」「超過時に `cancel()` する」という性質を名指しする形へ書き直した / Made the Worker's size cap bound what it reads, not only what it relays — and replaced tests that asserted "streaming" while only checking the return value
